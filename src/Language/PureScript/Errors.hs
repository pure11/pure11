...
prettyPrintSingleError full level e = prettyPrintErrorMessage <$> onTypesInErrorMessageM replaceUnknowns (if full then e else simplifyErrorMessage e)
 where
  ...
  prettyPrintErrorMessage em =
    ...
    go (SimpleErrorWrapper sem) = indent $ goSimple sem
...
indent b = Box.moveUp 1 $ Box.moveDown 1 $ Box.hsep 1 Box.left [ gutter, b ]
  where
  gutter = Box.vcat Box.left (replicate (Box.rows b) (Box.char '>'))
