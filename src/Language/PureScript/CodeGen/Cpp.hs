...
moduleToCpp env (Module _ mn imps _ foreigns decls) = do
  ...
  declToCpp vqs (_, ident) (Abs (_, com, ty, _) arg body) = do
    fn <- if CppTopLevel `elem` vqs
            then do
              ...
              block <- asReturnBlock <$>
                       if isAccessor absBody && not (null argNames)
                         then flip CppApp (CppVar <$> argNames) <$> valueToCpp vqs absBody
                         else valueToCpp vqs
                                (if null argNames
                                   then absBody
                                   else curriedApp
  ...
  -------------------------------------------------------------------------------------------------
  valueToCpp _ (Var (_, _, _, Just IsNewtype) ident) =
...
---------------------------------------------------------------------------------------------------
curriedLambda args ret = asReturnBlock $
                           foldr (\arg ret' -> CppLambda
                                                   [CppCaptureAll]
